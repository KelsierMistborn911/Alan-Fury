...
public class NaturePlacement : MonoBehaviour, CollectablePlant.IInstanceRemover
{
    ...
    public void Init()
    {
        ...
        foreach (var layer in layers)
        {
            ...
            {
                if (v == null) continue;
                if (!ResolveMeshAndMaterial(v))
                {
                    Debug.LogWarning($"NaturePlacement: '{layer.name}/{v.name}' — нет mesh/material, пропуск.");
                    continue;
                }
        ...
        _ready = true;
        Debug.Log($"NaturePlacement: Init OK, variants={allVariants.Count}");
    }
    ...
    private void GenerateSector(int sx, int sz)
    {
        ...
        for (int vi = 0; vi < allVariants.Count; vi++)
        {
            ...
            if (v.resolvedMesh == null || v.material == null) continue;
    ...
    private bool ResolveMeshAndMaterial(NatureVariant v)
    {
        if (v.mesh != null)
        {
            v.resolvedMesh = v.mesh;
            if (v.material != null) EnsureInstancing(v.material);
            return v.material != null;
        }
        if (v.prefab != null)
        {
            var mf = v.prefab.GetComponentInChildren<MeshFilter>();
            if (mf != null && mf.sharedMesh != null)
            {
                v.resolvedMesh = mf.sharedMesh;
                if (v.material == null)
                {
                    var mr = mf.GetComponent<MeshRenderer>();
                    if (mr != null) v.material = mr.sharedMaterial;
                }
                if (v.material != null) EnsureInstancing(v.material);
                return v.material != null;
            }
