...
/// <summary>
/// Лес HP1 / LOTR: зоны плотности, стена по краю, GPU-инстансы у игрока.
/// </summary>
public class ForestZoneRenderer : MonoBehaviour
{
    ...
    public class TreePart
    {
        ...
        public Matrix4x4 localToRoot; // relative to prefab root
    }
    ...
    public class TreeType
    {
        ...
        [Tooltip("Кинь префаб — все MeshFilter из иерархии снимутся сами.")]
        public GameObject prefab;
        // legacy single-mesh (filled from first part for scale calc)
        [HideInInspector] public Mesh mesh;
        [HideInInspector] public Material material;
        [HideInInspector] public int subMeshIndex = 0;

        [Header("Плотность внутри зоны (пресет перезаписывает)")]
        [Range(0f, 1f)] public float densityClear = 0.0f;
        [Range(0f, 1f)] public float densitySparse = 0.40f;  // роща
        [Range(0f, 1f)] public float densityGrove = 0.92f;   // обычный лес
        [Range(0f, 1f)] public float densityDense = 0.98f;   // чаща

        [Tooltip("Игнорируется: всегда 1 дерево на макро 5×5.")]
        public int treesPerCell = 1;

        [Header("Внешний вид")]
        public bool randomRotationY = true;
        [Tooltip("Относительный множитель высоты после подгонки (1 = пропорционально)")]
        public float minScale = 0.9f;
        public float maxScale = 1.2f;
        [Range(0.25f, 1f)] public float trunkThinness = 0.50f;
        [Tooltip("0 = авто от Tree Occupy Cells. Или задай вручную в мировых юнитах.")]
        public float targetFootprint = 0f;
        public float heightOffset = 0f;
        [Range(0f, 0.15f)] public float cellJitter = 0f;
        [Tooltip("Не используется: одно дерево на макро.")]
    ...
    [Header("Зоны леса (Роща / Лес / Чаща)")]
    [Tooltip("Чем меньше — тем крупнее цельные зоны. ~0.006 = большие области.")]
    public float zoneNoiseScale = 0.006f;
    public float detailNoiseScale = 0.03f;
    [Range(0f, 1f)] public float sparseThreshold = 0.32f;  // роща
    [Range(0f, 1f)] public float groveThreshold = 0.52f;   // лес
    [Range(0f, 1f)] public float denseThreshold = 0.72f;   // чаща
    ...
    [Tooltip("Минимальное расстояние между центрами деревьев (юниты). Больше = реже.")]
    public float treeSpacing = 14f;
    [Tooltip("Сколько клеток занимает одно дерево (и блокировка Pathfinder).")]
    [Range(1, 8)] public int treeOccupyCells = 4;
    [Tooltip("Случайный сдвиг от центра слота (доля от treeSpacing). 0 = строгая сетка, 0.25 = естественно.")]
    [Range(0f, 0.4f)] public float placementJitter = 0.18f;

    [Header("Секторы и прорисовка")]
    public int sectorSize = 16;
    [Tooltip("Базовый радиус прорисовки деревьев (м). При зуме-ауте увеличивается, при peek — фокус смещается.")]
    ...
    public bool showTreeMarkers = false;
    ...
    public Color borderColor = new Color(0.15f, 1f, 0.25f, 0.9f);
    public Color treeMarkerColor = new Color(0.9f, 1f, 0.3f, 1f);
    ...
    [ContextMenu("Apply Cinematic Preset (HP/LOTR)")]
    public void ApplyCinematicPreset()
    {
        zoneNoiseScale = 0.006f;      // крупные зоны, не патчи
        detailNoiseScale = 0.03f;
        sparseThreshold = 0.32f;      // роща
        groveThreshold = 0.52f;       // лес
        denseThreshold = 0.72f;       // чаща
        edgeWallCells = 16;
        edgeWallSoftCells = 10;       // чаща → лес → роща от края
        ...
        {
            // Лес (Grove) = обычный лес, густота как бывшая чаща
            // Чаща (Dense) = край, почти сплошная
            // Роща (Sparse) = опушки
            t.densityClear = 0.0f;
            t.densitySparse = 0.40f;   // роща
            t.densityGrove = 0.92f;    // обычный лес
            t.densityDense = 0.98f;    // чаща у края
            t.treesPerCell = 1;
            t.minScale = 0.9f;
            t.maxScale = 1.2f;
        ...
        zoneMap = null;
        Debug.Log("ForestZoneRenderer: preset — зоны сплошные, лес≈бывшая чаща, внутри spacing/occupy.");
    }
    ...
    public void PlaceForest()
    {
        ...
        foreach (var t in treeTypes)
        {
            ...
            grandTotal += count;
            Debug.Log($"ForestZoneRenderer: '{t.name}' — {count} деревьев, секторов {t.sectors.Count}");
        }

        isGenerated = true;
        Debug.Log($"ForestZoneRenderer: всего {grandTotal} деревьев, карта {_w}×{_d}");
    }
    ...
    private void BuildZoneMap(int w, int d)
    {
        ...
        // Решение по макро-блокам → крупные цельные зоны, без мелких патчей
        int macro = 1;
        if (heightSource != null && heightSource.macroSize > 1)
            macro = heightSource.macroSize;
        // ещё укрупняем: минимум ~8–12 клеток на зону
        ...
        for (int bx = 0; bx < bw; bx++)
        {
            for (int bz = 0; bz < bd; bz++)
            {
                ...
                // Край карты: Чаща → Лес → Роща (плавно)
                if (distEdge < hardWall)
                {
                    blockDens[bx, bz] = Density.Dense; // чаща
                    continue;
                }
                if (distEdge < edgeWallCells)
                {
                    ...
                    if (t < 0.4f) blockDens[bx, bz] = Density.Dense;      // чаща
                    else if (t < 0.75f) blockDens[bx, bz] = Density.Grove; // лес
                    else blockDens[bx, bz] = Density.Sparse;               // роща
                    continue;
                }

                // Интерьер: один низкочастотный шум → крупные области Чаща / Лес / Роща
                float n = Mathf.PerlinNoise(
                    (cx + noiseOffset.x) * zoneNoiseScale,
                    (cz + noiseOffset.y) * zoneNoiseScale);

                if (n >= denseThreshold) blockDens[bx, bz] = Density.Dense;       // чаща
                else if (n >= groveThreshold) blockDens[bx, bz] = Density.Grove; // лес
                else if (n >= sparseThreshold) blockDens[bx, bz] = Density.Sparse; // роща
                else blockDens[bx, bz] = Density.Clear;
            }
        }

        // Размазать блоки на клетки
        for (int x = 0; x < w; x++)
            for (int z = 0; z < d; z++)
            {
        ...
        // Переходы между зонами: полоса на границе Чаща↔Лес↔Роща
        ApplyZoneTransitions(w, d);
    }

    /// <summary>
    /// Смягчает жёсткие стыки: Dense|Grove → Grove на границе, Grove|Sparse → Sparse и т.п.
    /// </summary>
    private void ApplyZoneTransitions(int w, int d)
    {
        ...
        for (int x = 1; x < w - 1; x++)
        {
            for (int z = 1; z < d - 1; z++)
            {
                ...
                // на границе с более открытой зоной — ступень вниз (плавный переход)
                if (minN < cur)
                {
                    if (cur == Density.Dense && minN <= Density.Grove)
                        zoneMap[x, z] = Density.Grove;      // чаща → лес
                    else if (cur == Density.Grove && minN <= Density.Sparse)
                        zoneMap[x, z] = Density.Sparse;     // лес → роща
                    else if (cur == Density.Sparse && minN == Density.Clear)
                        zoneMap[x, z] = Density.Sparse;     // роща остаётся опушкой
                }
            }
        }
    }
                           ...
                           Dictionary<Vector2Int, List<Matrix4x4>> temp)
    {
        ...
        if (t.parts != null && t.parts.Count > 0)
        {
            ...
            foreach (var part in t.parts)
            {
                ...
                // transform mesh bounds by localToRoot (approx scale)
                Vector3 size = part.mesh.bounds.size;
                Vector3 lossy = part.localToRoot.lossyScale;
                size.x *= Mathf.Abs(lossy.x);
                size.y *= Mathf.Abs(lossy.y);
                size.z *= Mathf.Abs(lossy.z);
                if (first) { combined = new Bounds(Vector3.zero, size); first = false; }
                else combined.Encapsulate(new Bounds(Vector3.zero, size));
            }
        ...
        // ровная сетка с минимальным местом
        ...
        for (int bx = 0; bx < w; bx += step)
        {
            for (int bz = 0; bz < d; bz += step)
            {
                ...
                // центр слота + лёгкий джиттер (не хаос, а живой лес)
                ...
                // не вылезать на дорогу после джиттера
                ...
                // блокируем occupy×occupy клеток вокруг дерева
                int half = occupy / 2;
    ...
    void Update()
    {
        ...
        if (HasExternalDrawer) return; // ViewOcclusion рисует
        Vector3 focus = ResolveFocusPosition();
        if (float.IsInfinity(focus.x)) return;
        DrawAround(focus);
    }
    ...
    private Vector3 ResolveFocusPosition()
    {
        ...
        {
            if (player == null) player = PlayerRegistry.ResolvePrimary();
...
#endif
        return new Vector3(float.PositiveInfinity, 0f, 0f);
    }

    private void DrawAround(Vector3 focus)
    {
        if (_sectorWorld <= 0f && terrainBuilder != null)
            EnsureDrawCache();
        ...
        foreach (var t in treeTypes)
        {
            ...
            for (int sx = pcx - rad; sx <= pcx + rad; sx++)
            {
                for (int sz = pcz - rad; sz <= pcz + rad; sz++)
                {
                    ...
                    {
                        var rootBatch = batches[b];
                        // Для каждой части префаба — свой проход (с учётом localToRoot)
                        for (int p = 0; p < t.parts.Count; p++)
                        {
                            var part = t.parts[p];
                            if (part.mesh == null || part.material == null) continue;

                            // Собираем world matrices = root * localToRoot
                            var worldArr = new Matrix4x4[rootBatch.Length];
                            for (int i = 0; i < rootBatch.Length; i++)
                                worldArr[i] = rootBatch[i] * part.localToRoot;

                            Graphics.DrawMeshInstanced(
                                part.mesh, part.subMeshIndex, part.material,
                                worldArr, worldArr.Length, t.propertyBlock,
                                t.shadowCasting, t.receiveShadows);
                        }
                    }
                }
            }
    ...
    /// <summary>Для ViewOcclusion: обход видимых матриц вокруг focus (базовый drawRadius).</summary>
    ...
    /// <summary>
    /// Обход видимых матриц вокруг focus с опциональным радиусом (м).
    /// radiusOverride &lt; 0 → используется drawRadius.
    /// </summary>
    ...
    /// <summary>
    /// True when ViewOcclusion is active — it owns all tree drawing (multi-part + fade).
    /// Otherwise ForestZoneRenderer draws opaque instanced itself (no soft edge / occlusion).
    /// </summary>
    public bool HasExternalDrawer => ViewOcclusion.Instance != null && ViewOcclusion.Instance.isActiveAndEnabled;
    public void InvalidateExternalDrawerCache() { /* no-op — sectors stay valid */ }
    ...
    void OnDrawGizmos()
    {
        ...
        {
            Vector3 focus = ResolveFocusPosition();
            if (!float.IsInfinity(focus.x)) DrawAround(focus);
        }

        if (!showZoneGizmos && !showTreeMarkers) return;
        ...
        // показываем макро-клетки (те же, под которые масштабируются деревья)
        int step = gizmoStep;
    ...
    private bool Validate()
    {
        ...
        {
            Debug.LogError("ForestZoneRenderer: HeightMapGenerator не готов!");
            return false;
        }
        if (terrainBuilder == null)
        {
            Debug.LogError("ForestZoneRenderer: нет ChunkedTerrainBuilder!");
            return false;
        }
        if (treeTypes == null || treeTypes.Count == 0)
            Debug.LogWarning("ForestZoneRenderer: нет типов деревьев — добавь Prefab.");
        return true;
    }
    ...
    private static bool ResolveMeshAndMaterial(TreeType t)
    {
        ...
        if (t.prefab != null)
        {
            ...
            // fallback: SkinnedMeshRenderer
            if (t.parts.Count == 0)
            {
                var smrs = t.prefab.GetComponentsInChildren<SkinnedMeshRenderer>(true);
                foreach (var smr in smrs)
                {
                    if (smr == null || smr.sharedMesh == null) continue;
                    Matrix4x4 local = RelativeMatrix(root, smr.transform);
                    int subCount = smr.sharedMesh.subMeshCount;
                    Material[] mats = smr.sharedMaterials;
                    for (int s = 0; s < subCount; s++)
                    {
                        Material mat = (mats != null && s < mats.Length) ? mats[s] : smr.sharedMaterial;
                        if (mat == null) mat = CreateTestMaterial(t.name);
                        if (!mat.enableInstancing) mat.enableInstancing = true;
                        t.parts.Add(new TreePart
                        {
        ...
        // legacy single-mesh fields (for scale / old ViewOcclusion)
        if (t.parts.Count > 0)
        {
        ...
        if (t.parts.Count == 0)
        {
            Debug.LogWarning($"ForestZoneRenderer: '{t.name}' — нет mesh в префабе.");
            return false;
        }

        Debug.Log($"ForestZoneRenderer: '{t.name}' — {t.parts.Count} part(s) from prefab.");
        return true;
    }

    /// <summary>Local matrix of t relative to root (handles hierarchy).</summary>
